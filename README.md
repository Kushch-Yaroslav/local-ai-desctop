...
Перед началом диалога запустите сервис Ollama. Приложение использует только три зарегистрированные локальные модели: `qwen3.8:27b-q4_K_M`, `glm-4.7-flash:q4_K_M` и `gpt-oss:20b`. Сервис должен хранить веса через `OLLAMA_MODELS=/media/yaroslav/DATA/ollama`; приложение подключается к `http://127.0.0.1:11434` только как клиент и не управляет процессом Ollama. Для internal-only image worker используется `minicpm-v4.5:q4_K_M`; при необходимости его tag можно переопределить одной переменной окружения `LOCAL_AI_VISION_MODEL`.
