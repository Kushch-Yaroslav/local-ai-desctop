...
function ThinkingBlock({ content, timeline: recordedTimeline, streaming, activities = [], actionCount, createdAt, completedAt, now, onApproval, approvalSubmitting = false, onReadingOlder }: { content?: string; timeline?: import('../shared/types').ThinkingTimelineEvent[]; streaming: boolean; activities?: ToolActivity[]; actionCount?: number; createdAt?: string; completedAt?: string | null; now?: number; onApproval?: (decision: ApprovalDecision) => void; approvalSubmitting?: boolean; onReadingOlder?: () => void }) {
...
function durationLabel(start: string, end: string | null, now = Date.now()): string | null {
  const seconds = Math.max(0, Math.round(((end ? new Date(end).getTime() : now) - new Date(start).getTime()) / 1000));
  return seconds >= 60 ? `${Math.floor(seconds / 60)}м ${seconds % 60}с` : `${seconds}с`;
}
function structuredActionCount(activities: ToolActivity[]): number { return new Set(activities.filter((activity) => activity.kind !== 'progress').map((activity) => activity.id)).size; }
...
export function App() {
  ...
  const runFor = (run: AnalysisRun) => <ActivityTrace key={run.id} activities={run.actions} actionCount={structuredActionCount(run.actions)} status={run.status} createdAt={run.createdAt} completedAt={run.completedAt} active={false} />;
  return <div className="app-shell"><Sidebar /><main className="main"><Toolbar /><section ref={conversationRef} onScroll={updateFollowState} className="conversation">
    ...
    {messages.map((message) => {
      ...
      const displayRun = run ?? liveRun;
      const activities = message.id.startsWith('stream-') && isGenerating ? toolActivities : run?.actions ?? [];
      const actionCount = liveRun ? toolActivityCount : displayRun ? structuredActionCount(activities) : undefined;
      const body = editingId === message.id ? <MessageEditor text={editingText} onChange={setEditingText} onSave={() => { void (async () => { if (await editMessage(message, editingText)) setEditingId(null); })(); }} onCancel={() => setEditingId(null)} /> : <>{message.projectReferences?.length ? <MessageProjectReferences references={message.projectReferences} /> : null}{message.attachments?.length ? <MessageAttachments attachments={message.attachments} /> : null}{message.role === 'assistant' && (message.thinking || activities.length > 0) ? <ThinkingBlock content={message.thinking} timeline={message.thinkingTimeline} activities={activities} actionCount={actionCount} createdAt={displayRun?.createdAt} completedAt={displayRun?.completedAt} now={agentClock} streaming={message.id.startsWith('stream-') && isGenerating} onApproval={(decision) => void approveAction(decision)} approvalSubmitting={approvalSubmitting} onReadingOlder={() => { followStream.current = false; }} /> : null}{message.content ? <Markdown streaming={message.id.startsWith('stream-') && isGenerating}>{message.content}</Markdown> : message.role === 'assistant' && isGenerating && !message.thinking ? <GenerationIndicator state={generationState} /> : null}{message.role === 'assistant' && message.generationStats ? <GenerationStatsView stats={message.generationStats} /> : null}</>;
