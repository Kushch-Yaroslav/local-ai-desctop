...
export function Composer() {
  ...
  return <div className="composer-wrap"><div className="composer"><textarea ref={ref} value={value} placeholder="Напишите сообщение…" rows={1} onPointerDown={startResize} onPointerUp={finishResize} onChange={(event) => setValue(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); submit(); } }} /><ContextUsage />{isGenerating ? <button className="send-button stop" onClick={() => void stop()} title="Остановить генерацию"><Square size={16} fill="currentColor" /></button> : <button className="send-button" disabled={!value.trim()} onClick={submit} title="Отправить"><Send size={18} /></button>}</div><p>Enter — отправить · Shift+Enter — новая строка</p></div>;
}
