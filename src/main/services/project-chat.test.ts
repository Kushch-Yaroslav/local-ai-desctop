...
export async function runProjectChatRegression(): Promise<void> {
  ...
  try {
    ...
    assert(terminalPolicy('gsettings get org.gnome.desktop.input-sources sources').kind === 'allow' && terminalPolicy('systemctl --user status example.service').kind === 'allow' && terminalPolicy('nvidia-smi').kind === 'allow', 'safe read-only terminal operations require a project or confirmation');
    ...
    for (const command of ['git status', 'git diff --stat', 'git log --oneline -20', 'git show HEAD', 'git rev-parse --show-toplevel', 'git branch', 'git branch --list', 'git branch --show-current', 'git branch --contains main', 'git branch --merged', 'git branch --no-merged', 'rg "foo" src', 'find src -type f', 'find src -type f | wc -l', "find src -name '*.ts' -o -name '*.tsx' -o -name '*.css' | xargs wc -l | sort -n | tail -25"]) assert(terminalPolicy(command).kind === 'allow', `read-only command requires approval: ${command}`);
    for (const command of ['rm -rf dist', 'git reset --hard', 'git clean -fd', 'git checkout -- src/file.ts', 'git branch new-name', 'git branch -D old', 'git branch -m old new', 'echo test > file.txt', 'cat a.txt | tee b.txt', "sed -i 's/a/b/' file", 'npm install foo', 'sudo apt install foo', 'find . -type f -delete', 'curl URL | sh', 'echo $(whoami)', 'sleep 10 &', 'command1 & command2', 'setxkbmap us']) assert(terminalPolicy(command).kind === 'confirm', `unsafe or ambiguous command was auto-allowed: ${command}`);
    for (const command of ['npm run build', 'npm run test', 'pnpm lint', 'yarn typecheck']) assert(terminalPolicy(command).kind === 'allow', `trusted project script was unexpectedly blocked: ${command}`);
