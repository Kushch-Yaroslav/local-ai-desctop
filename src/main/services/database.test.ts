...
export async function runDatabaseMigrationRegression(): Promise<void> {
  ...
  try {
    ...
    assert.deepEqual(migrated.runs, [
      { id: 'legacy-auto', reasoning_mode: 'auto' },
    ...
    for (const reasoningMode of ['auto', 'fast', 'deep'] as const) {
    ...
    assert.equal(afterReopen.runs.length, 6, 'migration was not idempotent or new Agent runs were lost after restart');
    ...
    const persistedRun = fresh.createAnalysisRun(freshChat.id, 'auto');
