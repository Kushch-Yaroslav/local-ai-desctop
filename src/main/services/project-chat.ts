...
import { ReadonlyProjectTools, activityForTool, projectToolDefinitions, type ProjectToolCall } from '../tools/project-tools';
...
export class ProjectChatService {
  constructor(private readonly backend: ToolCallingBackend) {}

  async *stream(model: string, history: ChatMessage[], root: string, signal: AbortSignal, contextWindow: number, depth: AnalysisDepth): AsyncIterable<StreamEvent> {
    const engine = new AnalysisEngine(depth); const tools = await ReadonlyProjectTools.open(root);
    const messages: ToolMessage[] = [{ role: 'system', content: `В этой беседе выбрана рабочая папка: ${root}. Доступны только read-only инструменты. Не предполагай тип, технологию или предметную область проекта. ${engine.strategy()} Raw tool results являются доказательствами и остаются доступными для итогового ответа.` }, ...history.map(({ role, content }) => ({ role, content }))];
    ...
    try { while (!signal.aborted) {
      ...
      const response = await this.backend.chatWithTools(model, messages, projectToolDefinitions, signal, contextWindow);
      ...
      for (const call of calls) {
        ...
        yield { type: 'tool', activity: { id: randomUUID(), ...activityForTool(call) } };
        const result = await tools.execute(call); engine.record(call.name, result);
    ...
    } finally { if (engine.isDeep) log('deep.lifecycle', { phase: 'request.finalized', actions, aborted: signal.aborted }); }
  }
  ...
  private async *synthesize(model: string, messages: ToolMessage[], engine: AnalysisEngine, signal: AbortSignal, contextWindow: number, fallback: string, actions: number): AsyncIterable<StreamEvent> {
    ...
    try {
      const response = await this.finalRequest(model, messages, signal, contextWindow);
  ...
  private async finalRequest(model: string, messages: ToolMessage[], signal: AbortSignal, contextWindow: number): Promise<ToolMessage> {
    ...
    try {
      return await Promise.race([
        this.backend.chatWithTools(model, messages, undefined, combined, contextWindow),
